...
let selected=[],enabled=false,autostart=false,authenticated=false,hasRecords=false,uncertain=false,ticket=null,includeHistory=true,calledAvailable=true;
const stamp=new Date().toISOString();
const stores=[{id:1012,name:'深圳海岸城店',address:'深圳市南山区海德三道',wait:35},{id:3006,name:'深圳万象天地店',address:'深圳市南山区深南大道',wait:50}];
await page.route('**/api/**',async route=>{
  ...
  if(key==='/api/queue/service'){
  ...
  else if(key==='/api/status')data={version:'preview',has_config:authenticated,platform:'windows',engine:{status:'idle'},auth_health:{status:'unknown'}};
  else if(key==='/api/queue/stores')data={stores};
  else if(key==='/api/queue/live')data={store_id:'1012',store_name:stores[0].name,store_status:'OPEN',online_open:true,called_no:123,wait_groups:18,server_wait_minutes:35,observed_at:stamp};
    ...
    else{ticket={number:'A148',store_id:'1012'};data={ticket};}
  }else if(key==='/api/queue/ticket/status')data={ticket};
...
try{
  ...
  hasRecords=true;await click('refresh');await page.locator('#analysis-store').selectOption('1012');await page.locator('#record-chart svg').waitFor();
  ...
  uncertain=true;await click('review-ticket');await click('confirm-ticket');await page.getByText('取号结果还未确认',{exact:true}).waitFor();
